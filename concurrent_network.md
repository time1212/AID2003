...
* 服务端（Server）：服务端是为客户端服务的，服务的内容诸如向客户端提供资源，保存客户端数据，处理客户端请求等。**端口由程序员决定**

* 客户端（Client） ：也称为用户端，是指与服务端相对应，为客户提供一定应用功能的程序，我们平时使用的手机或者电脑上的程序基本都是客户端程序。**端口由操作系统分配**
...
* 什么是IO
  ...
  在程序中存在读写数据操作行为的事件均是IO行为，比如**终端输入输出 ,文件读写，数据库修改和网络消息收发**等。
